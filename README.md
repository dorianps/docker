...
Tools to create and run docker containers with pre-installed ANTsR, LINDA, or LESYMAP R packages. The RStudio GUI is accessed through the browser, and several other developer tools are integrated. The docker images built with these scripts are available in  [DockerHub](https://hub.docker.com/u/dorianps), but you can build new images locally.
